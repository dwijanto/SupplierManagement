...
Public Class FormMyTaskDocument
    ...
    Sub DoWork()
        ...
        sb.Append(String.Format("select * from (select * from (with vd as (" &
                  " select distinct headerid,status from doc.vendordoc where not status isnull )" &
                  " Select h.id, 'New'::text as statusname,h.creationdate,o.userid::character varying as username,doc.sp_getshortname(h.id) as shortname,doc.sp_getsuppliers(h.id) as suppliername ,doc.sp_getvendorcode(h.id)::text as vendorcode, null::text as vstatusname,doc.sp_getdoctypename(h.id)::character varying as doctypename,null::character varying as projectname,null::character varying as version, null::date as docdate,null::date as expireddate,null::integer as statusexp,o2.officersebname::character varying as validatorname,o3.officersebname::character varying as cc1name ,o4.officersebname::character varying as cc2name,o5.officersebname::character varying as cc3name,o6.officersebname::character varying as cc4name ,h.userid,h.validator,h.cc1,h.cc2,h.cc3,h.cc4,h.otheremail,h.latestupdate,Null::integer as doctypeid,0::bigint as vdid,0::bigint as documentid" &
                  " from doc.header h " &
                  " left join doc.user o on lower(o.userid) = h.userid" &
                  " left join officerseb o2 on lower(o2.userid) = h.validator" &
                  " left join officerseb o3 on lower(o3.userid) = h.cc1" &
        ...
        '          " (hd.userid = '" & myuser & "' or hd.validator = '" & myuser & "' or hd.cc1 = '" & myuser & "' or hd.cc2 = '" & myuser & "' or hd.cc3 = '" & myuser & "' or hd.cc4 = '" & myuser & "'  or " & (HelperClass1.UserInfo.IsAdmin) & " or " & (HelperClass1.UserInfo.AllowUpdateDocument) & ")  and not hd.id isnull) as foo {0} ;", CriteriaNew))
        sb.Append(String.Format(" select * from (with vst as(select paramname as statusname ,ivalue as id from doc.paramdt where paramhdid = 2)" &
