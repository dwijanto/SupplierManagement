...
Public Class FormDocumentVendor4
    ...
    Sub DoWork()
        ...
        sb.Append("select vd.id as vdid,vd.vendorcode,vd.documentid,vd.status,doc.getstatusdoc(vd.status) as statusname,vd.headerid,v.vendorname::text,v.shortname::text,d.*,vr.version,gt.paymentcode,sc.leadtime,sc.sasl,q.nqsu,p.projectname,sa.auditby,sa.audittype,sa.auditgrade,sa.overallauditresult,sa.score as auditscore,sef.score,sif.myyear,sif.turnovery,sif.turnovery1,sif.turnovery2,sif.turnovery3,sif.turnovery4,sif.ratioy,sif.ratioy1,sif.ratioy2,sif.ratioy3,sif.ratioy4,null as filename,dt.doctypename,dl.levelname,de.expireddate from doc.vendordoc vd" &
        ...
        sb.Append("select ''::text as name,'' as userid,null as teamtitleid,'' as officersebname,''::text as teamtitleshortname union all (select distinct teamtitleshortname || ' - ' || mu.username as name,lower(o.userid) as userid," &
                  " tt.teamtitleid,mu.username as officersebname,tt.teamtitleshortname from doc.user u left join officerseb o on o.userid = u.userid  left join teamtitle tt on tt.teamtitleid = o.teamtitleid left join masteruser mu on mu.id = o.muid where teamtitleshortname in ('PD','SPM','PM','PO') and mu.isactive order by tt.teamtitleid,mu.username);")
    ...
    Private Sub DataGridView4_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView4.CellDoubleClick, DataGridView5.CellDoubleClick, DataGridView6.CellDoubleClick
