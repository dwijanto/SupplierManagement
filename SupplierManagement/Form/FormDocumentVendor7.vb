...
Public Class FormDocumentVendor7
    ...
    Dim bsZetol As BindingSource
    Dim bsZetolMaster As BindingSource
    Dim bsGO As BindingSource
    Dim bsSO As BindingSource
    Dim bsQO As BindingSource
    ...
    Dim myTx As TxRecord
    Dim bsSiTx As BindingSource
    Dim bsActionPlan As BindingSource
    ...
    Dim ShortNameTMP As String = String.Empty
            ...
            Try
                ...
                If Not IsNothing(ds2) Then
                    ...
                    If DbAdapter1.DocumentVendorTx(Me, mye) Then
                        ...
                        'Do not change otherwise you'll get error
                        For i = 0 To 1
                ...
                Select Case drv.Row.Item("doctypename")
                    ...
                    Case "Social Audit"
                        If IsDBNull(drv.Row.Item("auditby")) Or IsDBNull(drv.Row.Item("audittype")) Or IsDBNull(drv.Row.Item("auditgrade")) Then
                            ...
                            myret = False
                        End If
                        If drv.Row.Item("auditby") = "Intertek" Or drv.Row.Item("auditby") = "Waived Audit" Then
                            If IsDBNull(drv.Row.Item("overallauditresult")) Then
                                myerror.Append("Overall Audit Result cannot be blank!")
                                myret = False
                            End If
                        End If

                        If Not IsDBNull(drv.Row.Item("overallauditresult")) Then
                            If IsDBNull(drv.Row.Item("auditscore")) Then
                                myerror.Append("Overall Audit Score cannot be blank!")
                                myret = False
                            End If
                        End If
                        'Zero Tolerance NC", "Red Level",
                        If drv.Row.Item("auditby") = "Intertek" And (drv.Row.Item("auditgrade") = "Red Level" Or drv.Row.Item("auditgrade") = "Zero Tolerance NC") Then
                            If IsNothing(bsZetol.Current) Then
                                myerror.Append("Zero Tolerance/Red Level Section cannot be blank!")
                                myret = False
                            End If
                        End If
    ...
    Sub DoWork()
        ...
        'sb.Append("select vd.id as vdid,vd.vendorcode,vd.documentid,vd.status,doc.getstatusdoc(vd.status) as statusname,vd.headerid,v.vendorname::text,v.shortname::text,d.*," &
        '          " vr.version,gt.paymentcode,sc.leadtime,sc.sasl,q.nqsu,p.projectname,sa.auditby,sa.audittype,sa.auditgrade,sa.overallauditresult,sa.score as auditscore,sef.score," &
        '          " sif.myyear,sif.turnovery,sif.turnovery1,sif.turnovery2,sif.turnovery3,sif.turnovery4,sif.ratioy,sif.ratioy1,sif.ratioy2,sif.ratioy3,sif.ratioy4,null as filename,dt.doctypename,dl.levelname,de.expireddate," &
        '         " bi.producttype,bi.lastvisit,bi.keystakestopic,bi.strength1,bi.strength2,bi.strength3,bi.weaknessess1,bi.weaknessess2,bi.weaknessess3,bi.opportunities1," &
        '         " bi.opportunities2,bi.opportunities3,bi.threats1,bi.threats2,bi.threats3,bi.myyear as myyear2,bi.currbudget,bi.totalbudget,bi.productdevelopment1,bi.productdevelopment2," &
        '         " bi.negotiationresult1,bi.negotiationresult2,bi.negotiationresult3,bi.outstandingissue1,bi.outstandingissue2,bi.outstandingissue3,bi. outstandingissue4, bi.outstandingissue5" &
        ...
        '         " Left join doc.buyerinput bi on bi.documentid = d.id" &
        '         " where h.id = " & headerid & ";")
        sb.Append("select vd.id as vdid,vd.vendorcode,vd.documentid,vd.status,doc.getstatusdoc(vd.status) as statusname,vd.headerid,v.vendorname::text,v.shortname::text,d.*," &
          " vr.version,gt.paymentcode,sc.leadtime,sc.sasl,q.nqsu,p.projectname,sa.auditby,sa.audittype,sa.auditgrade,sa.overallauditresult,sa.score as auditscore,sef.score," &
          " sif.myyear,sif.turnovery,sif.turnovery1,sif.turnovery2,sif.turnovery3,sif.turnovery4,sif.ratioy,sif.ratioy1,sif.ratioy2,sif.ratioy3,sif.ratioy4,null as filename,dt.doctypename,dl.levelname,de.expireddate," &
         " bi.producttype,bi.lastvisit,bi.keystakestopic,bi.strength1,bi.strength2,bi.strength3,bi.weaknessess1,bi.weaknessess2,bi.weaknessess3,bi.opportunities1," &
         " bi.opportunities2,bi.opportunities3,bi.threats1,bi.threats2,bi.threats3,bi.myyear as myyear2,bi.currbudget,bi.totalbudget,bi.productdevelopment1,bi.productdevelopment2," &
         " bi.negotiationresult1,bi.negotiationresult2,bi.negotiationresult3,bi.outstandingissue1,bi.outstandingissue2,bi.outstandingissue3,bi. outstandingissue4, bi.outstandingissue5,bi.keycontactperson," &
         " bi.title,bi.top3cust1,bi.top3cust2,bi.top3cust3,ps.returnrate" &
         " from doc.vendordoc vd" &
         " left join doc.header h on h.id = vd.headerid" &
         " left join vendor v on v.vendorcode = vd.vendorcode" &
         " left join doc.document d on d.id = vd.documentid" &
        ...
        sb.Append("with zetol as (select pd.paramdtid as id, pd.paramname as name from doc.paramhd ph " &
                  ...
                  " select * from zetol ;")
        'ZetolTx
        sb.Append("with zt as (select pd.paramdtid as id, pd.paramname as name,pd.ivalue from doc.paramhd ph " &
                  " left join doc.paramdt pd on pd.paramhdid = ph.paramhdid" &
                  " where ph.paramname = 'zetol' order by pd.ivalue)" &
                  "select z.*,zt.name as zename,vd.id as vdid from doc.zetol z" &
                  " left join doc.vendordoc vd on vd.documentid = z.documentid" &
                  " left join zt on zt.id = z.zetolid" &
                  " where vd.headerid = " & headerid & " order by zt.ivalue;")
        'GeneralContractOther
        sb.Append(String.Format("select go.*,vd.id as vdid ,payt || ' - ' || details as payt from doc.generalcontractother go left join doc.vendordoc vd on vd.documentid = go.documentid " &
                  " left join paymentterm pt on pt.paymenttermid = go.paymentcode" &
                  " where vd.headerid = {0} order by go.otherdate;", headerid))
        'SupplyChainOther
        sb.Append(String.Format("select so.*,vd.id as vdid,'LeadTime' as leadtimehd,'SASL' as saslhd from doc.supplychainother so left join doc.vendordoc vd on vd.documentid = so.documentid " &
                                " where vd.headerid = {0} order by so.otherdate;", headerid))
        'QualityAppendixOther
        sb.Append(String.Format("select qo.*,vd.id as vdid from doc.qualityappendixother qo left join doc.vendordoc vd on vd.documentid = qo.documentid " &
                  " where vd.headerid = {0} order by qo.otherdate;", headerid))

        'Action Plan
        sb.Append(String.Format("select ap.*,vd.id as vdid from doc.actionplan ap " &
                                " left join doc.vendordoc vd on vd.documentid = ap.documentid  " &
                                " where vd.headerid = {0} order by ap.id;", headerid))
        If DbAdapter1.TbgetDataSet(sb.ToString, DS, mymessage) Then
            Try
                ...
                DS.Tables(11).TableName = "ZetolMaster"
                DS.Tables(12).TableName = "ZetolTx"
                DS.Tables(13).TableName = "GOTx"
                DS.Tables(14).TableName = "SOTx"
                DS.Tables(15).TableName = "QOTx"
                DS.Tables(16).TableName = "ActionPlanTx"
            Catch ex As Exception
                ...
                Select Case id
                    ...
                    Case 4
                        Try
                            ...
                            bsZetol = New BindingSource
                            bsGO = New BindingSource
                            bsSO = New BindingSource
                            bsQO = New BindingSource
                            ...
                            bsZetolMaster = New BindingSource
                            bsActionPlan = New BindingSource
                            ...
                            pk1(0) = DS.Tables(1).Columns("vdid")
                            ...
                            'create relation with other table
                            ...
                            hcol = DS.Tables(1).Columns("vdid")
                            dcol = DS.Tables(12).Columns("vdid")
                            rel = New DataRelation("zetrel", hcol, dcol)
                            DS.Relations.Add(rel)

                            hcol = DS.Tables(1).Columns("vdid")
                            dcol = DS.Tables(13).Columns("vdid")
                            rel = New DataRelation("gorel", hcol, dcol)
                            DS.Relations.Add(rel)

                            hcol = DS.Tables(1).Columns("vdid")
                            dcol = DS.Tables(14).Columns("vdid")
                            rel = New DataRelation("sorel", hcol, dcol)
                            DS.Relations.Add(rel)

                            hcol = DS.Tables(1).Columns("vdid")
                            dcol = DS.Tables(15).Columns("vdid")
                            rel = New DataRelation("qorel", hcol, dcol)
                            DS.Relations.Add(rel)

                            hcol = DS.Tables(1).Columns("vdid")
                            dcol = DS.Tables(16).Columns("vdid")
                            rel = New DataRelation("aprel", hcol, dcol)
                            DS.Relations.Add(rel)

                            bsheader.DataSource = DS.Tables(0)
                            bsDetail.DataSource = DS.Tables(1)
                            bsZetol.DataSource = DS.Tables(12)
                            bsGO.DataSource = DS.Tables(13)
                            bsSO.DataSource = DS.Tables(14)
                            bsQO.DataSource = DS.Tables(15)
                            bsActionPlan.DataSource = DS.Tables(16)
                            ...
                            bsZetolMaster.DataSource = DS.Tables(11)
                            ...
                            UC = New UniqueConstraint(New DataColumn() {DS.Tables("zetoltx").Columns("documentid"),
                                                                        DS.Tables("zetoltx").Columns("vdid"),
                                                                        DS.Tables("zetoltx").Columns("zetolid")})
                            If Not IsNothing(UC) Then
                                DS.Tables("zetoltx").Constraints.Add(UC)
                            End If

                            Dim pk13(1) As DataColumn
                            pk13(0) = DS.Tables(13).Columns("id")
                            pk13(1) = DS.Tables(13).Columns("vdid")
                            DS.Tables(13).PrimaryKey = pk13
                            DS.Tables(13).Columns(0).AutoIncrement = True
                            DS.Tables(13).Columns(0).AutoIncrementSeed = 0
                            DS.Tables(13).Columns(0).AutoIncrementStep = -1

                            Dim pk14(1) As DataColumn
                            pk14(0) = DS.Tables(14).Columns("id")
                            pk14(1) = DS.Tables(14).Columns("vdid")
                            DS.Tables(14).PrimaryKey = pk14
                            DS.Tables(14).Columns(0).AutoIncrement = True
                            DS.Tables(14).Columns(0).AutoIncrementSeed = 0
                            DS.Tables(14).Columns(0).AutoIncrementStep = -1

                            Dim pk15(1) As DataColumn
                            pk15(0) = DS.Tables(15).Columns("id")
                            pk15(1) = DS.Tables(15).Columns("vdid")
                            DS.Tables(15).PrimaryKey = pk15
                            DS.Tables(15).Columns(0).AutoIncrement = True
                            DS.Tables(15).Columns(0).AutoIncrementSeed = 0
                            DS.Tables(15).Columns(0).AutoIncrementStep = -1

                            Dim pk16(1) As DataColumn
                            pk16(0) = DS.Tables(16).Columns("id")
                            pk16(1) = DS.Tables(16).Columns("vdid")
                            DS.Tables(16).PrimaryKey = pk16
                            DS.Tables(16).Columns(0).AutoIncrement = True
                            DS.Tables(16).Columns(0).AutoIncrementSeed = 0
                            DS.Tables(16).Columns(0).AutoIncrementStep = -1

                            Dim docid(1) As DataColumn
                            Dim zedocid(1) As DataColumn
                            docid(0) = DS.Tables(1).Columns("vdid")
                            docid(1) = DS.Tables(1).Columns("documentid")
                            zedocid(0) = DS.Tables(12).Columns("vdid")
                            zedocid(1) = DS.Tables(12).Columns("documentid")
                            rel = New DataRelation("doze", docid, zedocid)
                            FkeyConstraint = New ForeignKeyConstraint("fkey1", docid, zedocid)
                            FkeyConstraint.DeleteRule = Rule.Cascade
                            FkeyConstraint.UpdateRule = Rule.Cascade
                            'Check DocumentType for zetol
                            DS.Relations.Add(rel)

                            'GeneralcontralOther
                            Dim godocid(1) As DataColumn
                            godocid(0) = DS.Tables(13).Columns("vdid")
                            godocid(1) = DS.Tables(13).Columns("documentid")
                            rel = New DataRelation("dogo", docid, godocid)
                            FkeyConstraint = New ForeignKeyConstraint("fkey2", docid, godocid)
                            FkeyConstraint.DeleteRule = Rule.Cascade
                            FkeyConstraint.UpdateRule = Rule.Cascade
                            DS.Relations.Add(rel)

                            'SupplychainOther
                            Dim sodocid(1) As DataColumn
                            sodocid(0) = DS.Tables(14).Columns("vdid")
                            sodocid(1) = DS.Tables(14).Columns("documentid")
                            rel = New DataRelation("doso", docid, sodocid)
                            FkeyConstraint = New ForeignKeyConstraint("fkey3", docid, sodocid)
                            FkeyConstraint.DeleteRule = Rule.Cascade
                            FkeyConstraint.UpdateRule = Rule.Cascade
                            DS.Relations.Add(rel)

                            'QualityAppendixOther
                            Dim qodocid(1) As DataColumn
                            qodocid(0) = DS.Tables(15).Columns("vdid")
                            qodocid(1) = DS.Tables(15).Columns("documentid")
                            rel = New DataRelation("doqo", docid, qodocid)
                            FkeyConstraint = New ForeignKeyConstraint("fkey4", docid, qodocid)
                            FkeyConstraint.DeleteRule = Rule.Cascade
                            FkeyConstraint.UpdateRule = Rule.Cascade
                            DS.Relations.Add(rel)

                            'ActionPlan
                            Dim apdoid(1) As DataColumn
                            apdoid(0) = DS.Tables(16).Columns("vdid")
                            apdoid(1) = DS.Tables(16).Columns("documentid")
                            rel = New DataRelation("doap", docid, apdoid)
                            FkeyConstraint = New ForeignKeyConstraint("fkey5", docid, apdoid)
                            FkeyConstraint.DeleteRule = Rule.Cascade
                            FkeyConstraint.UpdateRule = Rule.Cascade
                            DS.Relations.Add(rel)
                            ...
                            'ComboBox1.DataBindings.Add("SelectedValue", bsDetail, "shortname") 'Never bind!! only combobox2 need to be binded. only for input helper purpose
                            ...
                            ComboBox9.DataBindings.Add("Text", bsDetail, "overallauditresult", True, DataSourceUpdateMode.OnPropertyChanged, "")

                            DataGridView3.AutoGenerateColumns = False
                            DataGridView3.DataSource = bsDetail
                            DataGridView3.DataMember = "zetrel"

                            DataGridView4.AutoGenerateColumns = False
                            DataGridView4.DataSource = bsDetail
                            DataGridView4.DataMember = "gorel"

                            DataGridView5.AutoGenerateColumns = False
                            DataGridView5.DataSource = bsDetail
                            DataGridView5.DataMember = "sorel"

                            DataGridView6.AutoGenerateColumns = False
                            DataGridView6.DataSource = bsDetail
                            DataGridView6.DataMember = "qorel"

                            DataGridView7.AutoGenerateColumns = False
                            DataGridView7.DataSource = bsDetail
                            DataGridView7.DataMember = "aprel"
                            ...
                            TextBox15.DataBindings.Add(New Binding("Text", bsDetail, "auditscore", True, DataSourceUpdateMode.OnPropertyChanged, ""))
                            TextBox16.DataBindings.Add(New Binding("Text", bsDetail, "returnrate", True, DataSourceUpdateMode.OnPropertyChanged, ""))
                            ...
                            Dim b29 As Binding = New Binding("Text", bsDetail, "producttype", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b30 As Binding = New Binding("Text", bsDetail, "lastvisit", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b31 As Binding = New Binding("Text", bsDetail, "keystakestopic", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b32 As Binding = New Binding("Text", bsDetail, "strength1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b33 As Binding = New Binding("Text", bsDetail, "strength2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b34 As Binding = New Binding("Text", bsDetail, "strength3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b35 As Binding = New Binding("Text", bsDetail, "weaknessess1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b36 As Binding = New Binding("Text", bsDetail, "weaknessess2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b37 As Binding = New Binding("Text", bsDetail, "weaknessess3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b38 As Binding = New Binding("Text", bsDetail, "opportunities1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b39 As Binding = New Binding("Text", bsDetail, "opportunities2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b40 As Binding = New Binding("Text", bsDetail, "opportunities3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b41 As Binding = New Binding("Text", bsDetail, "threats1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b42 As Binding = New Binding("Text", bsDetail, "threats2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b43 As Binding = New Binding("Text", bsDetail, "threats3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b44 As Binding = New Binding("Text", bsDetail, "myyear2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b45 As Binding = New Binding("Text", bsDetail, "currbudget", True, DataSourceUpdateMode.OnPropertyChanged, "", "#,##0.00")
                            Dim b46 As Binding = New Binding("Text", bsDetail, "totalbudget", True, DataSourceUpdateMode.OnPropertyChanged, "", "#,##0.00")
                            Dim b47 As Binding = New Binding("Text", bsDetail, "productdevelopment1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b48 As Binding = New Binding("Text", bsDetail, "productdevelopment2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b49 As Binding = New Binding("Text", bsDetail, "negotiationresult1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b50 As Binding = New Binding("Text", bsDetail, "negotiationresult2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b51 As Binding = New Binding("Text", bsDetail, "negotiationresult3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b52 As Binding = New Binding("Text", bsDetail, "outstandingissue1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b53 As Binding = New Binding("Text", bsDetail, "outstandingissue2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b54 As Binding = New Binding("Text", bsDetail, "outstandingissue3", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b55 As Binding = New Binding("Text", bsDetail, "outstandingissue4", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b56 As Binding = New Binding("Text", bsDetail, "outstandingissue5", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b57 As Binding = New Binding("Text", bsDetail, "keycontactperson", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b58 As Binding = New Binding("Text", bsDetail, "title", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b59 As Binding = New Binding("Text", bsDetail, "top3cust1", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b60 As Binding = New Binding("Text", bsDetail, "top3cust2", True, DataSourceUpdateMode.OnPropertyChanged, "")
                            Dim b61 As Binding = New Binding("Text", bsDetail, "top3cust3", True, DataSourceUpdateMode.OnPropertyChanged, "")


                            UcsssBuyerInput1.TextBox1.DataBindings.Add(b29)
                            'UcsssBuyerInput1.DateTimePicker1.DataBindings.Add(b30)
                            'UcsssBuyerInput1.TextBox3.DataBindings.Add(b31)
                            UcsssBuyerInput1.TextBox4.DataBindings.Add(b32)
                            UcsssBuyerInput1.TextBox5.DataBindings.Add(b33)
                            UcsssBuyerInput1.TextBox6.DataBindings.Add(b34)
                            UcsssBuyerInput1.TextBox7.DataBindings.Add(b35)
                            UcsssBuyerInput1.TextBox8.DataBindings.Add(b36)
                            UcsssBuyerInput1.TextBox9.DataBindings.Add(b37)
                            UcsssBuyerInput1.TextBox10.DataBindings.Add(b38)
                            UcsssBuyerInput1.TextBox11.DataBindings.Add(b39)
                            UcsssBuyerInput1.TextBox12.DataBindings.Add(b40)
                            UcsssBuyerInput1.TextBox13.DataBindings.Add(b41)
                            UcsssBuyerInput1.TextBox14.DataBindings.Add(b42)
                            UcsssBuyerInput1.TextBox15.DataBindings.Add(b43)
                            UcsssBuyerInput1.TextBox16.DataBindings.Add(b44)
                            UcsssBuyerInput1.TextBox17.DataBindings.Add(b45)
                            UcsssBuyerInput1.TextBox18.DataBindings.Add(b46)
                            UcsssBuyerInput1.TextBox19.DataBindings.Add(b47)
                            UcsssBuyerInput1.TextBox20.DataBindings.Add(b48)
                            UcsssBuyerInput1.TextBox21.DataBindings.Add(b49)
                            UcsssBuyerInput1.TextBox22.DataBindings.Add(b50)
                            UcsssBuyerInput1.TextBox23.DataBindings.Add(b51)
                            UcsssBuyerInput1.TextBox24.DataBindings.Add(b52)
                            UcsssBuyerInput1.TextBox25.DataBindings.Add(b53)
                            UcsssBuyerInput1.TextBox26.DataBindings.Add(b54)
                            UcsssBuyerInput1.TextBox27.DataBindings.Add(b55)
                            UcsssBuyerInput1.TextBox28.DataBindings.Add(b56)
                            UcsssBuyerInput1.TextBox2.DataBindings.Add(b57)
                            UcsssBuyerInput1.TextBox3.DataBindings.Add(b58)
                            UcsssBuyerInput1.TextBox29.DataBindings.Add(b59)
                            UcsssBuyerInput1.TextBox30.DataBindings.Add(b60)
                            UcsssBuyerInput1.TextBox31.DataBindings.Add(b61)
                            ...
                            If Not IsNothing(RenewDRV) Then
                                ...
                                Else 'Non Renew
                                    ...
                                    Dim itemfound As Integer = bsDetail.Find("vdid", RenewDRV.Item("vdid"))
                            ...
                            Else
                                ...
                                If recPosition >= 0 Then
                                    ...
                                    DataGridView1.Rows(recPosition).Selected = True
                                    DataGridView1.FirstDisplayedScrollingRowIndex = recPosition
                                End If
                            End If
    ...
    Private Sub bsDetail_ListChanged(ByVal sender As Object, ByVal e As System.ComponentModel.ListChangedEventArgs) Handles bsDetail.ListChanged
        ...
        If Not IsNothing(bsDetail.Current) Then
            ...
            Else
                ...
                ComboBox1.Text = "" & drv.Item("shortname")
            End If
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton1.Click
        Try
            ...
            mydrv.Row.Item("documentid") = mydrv.Row.Item("vdid")
    ...
    Private Sub enabledTextBox()
        ...
        ImportBuyerInput.Enabled = False
        ImportActionPlan.Enabled = False
        UcsssBuyerInput1.Enabled = False
        ...
        If Not IsNothing(ComboBox3.SelectedValue) Then
            ...
            Select Case drv.Row.Item("doctypename") 'ComboBox3.SelectedText
                ...
                Case "Identity sheet"
                    ...
                    ''TextBox27.Text = ""
                    ''TextBox28.Text = ""
                Case "Supplier Summary Sheet (SSS)"
                    TabControl1.SelectedTab = TabPage3
                    ImportBuyerInput.Enabled = True
                    ImportActionPlan.Enabled = True
                    ComboBox5.SelectedIndex = -1

                    UcsssBuyerInput1.Enabled = True
    ...
    Private Sub ComboBox3_SelectionChangeCommitted(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox3.SelectionChangeCommitted
        ...
        If DataGridView7.RowCount > 1 Then
            MessageBox.Show("If you want to change the document  type, please delete the row first.")
    ...
    Private Sub ComboBox2_SelectionChangeCommitted(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox2.SelectionChangeCommitted
        ...
        If Not IsNothing(ComboBox2.SelectedItem) Then
            ...
            If Not IsNothing(bsDetail.Current) Then
                ...
                mydrv.Row.Item("vendorname") = drv.Row.Item("vendorname")
                mydrv.Row.Item("shortname") = DBNull.Value
                ShortNameTMP = drv.Row.Item("shortname")
                bsDetail.EndEdit()
                DataGridView1.Invalidate()
            End If
            'End If
        ...
        Select Case cm.SourceControl.Name
            Case "DataGridView3"
                ...
                If myform.ShowDialog() = Windows.Forms.DialogResult.OK Then
                    Dim bsdetailcurrent As DataRowView = bsDetail.Current
                    For Each mydrv As DataRowView In myform.SelectedDrv
                        Dim drv As DataRowView = bsZetol.AddNew
                        Try
                            drv.Item("documentid") = bsdetailcurrent.Item("documentid")
                            drv.Item("vdid") = bsdetailcurrent.Item("vdid")
                ...
                End If
            Case "DataGridView4"
                AddUpdateGO(TxRecord.TxAdd)

                'MessageBox.Show("DataGridView4")
            Case "DataGridView5"
                AddUpdateSO(TxRecord.TxAdd)
            Case "DataGridView6"
                AddUpdateQO(TxRecord.TxAdd)
    ...
    Private Sub DeleteItemToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteItemToolStripMenuItem.Click
        ...
        Select Case cm.SourceControl.Name
            ...
            Case "DataGridView4"
                If MessageBox.Show("Delete this record?", "Delete Record", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.OK Then
                    For Each dgdrv As DataGridViewRow In DataGridView4.SelectedRows
                        Dim drv As DataRowView = dgdrv.DataBoundItem
                        bsGO.Position = bsGO.Find("vdid", drv.Item("vdid"))
                        bsGO.RemoveCurrent()
            ...
            Case "DataGridView5"
                If MessageBox.Show("Delete this record?", "Delete Record", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.OK Then
                    For Each dgdrv As DataGridViewRow In DataGridView5.SelectedRows
                        Dim drv As DataRowView = dgdrv.DataBoundItem
                        bsSO.Position = bsSO.Find("vdid", drv.Item("vdid"))
                        bsSO.RemoveCurrent()
                        DataGridView5.Invalidate()
                    Next
                End If
            Case "DataGridView6"
                If MessageBox.Show("Delete this record?", "Delete Record", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.OK Then
                    For Each dgdrv As DataGridViewRow In DataGridView6.SelectedRows
                        Dim drv As DataRowView = dgdrv.DataBoundItem
                        bsQO.Position = bsQO.Find("vdid", drv.Item("vdid"))
                        bsQO.RemoveCurrent()
                        DataGridView6.Invalidate()
                    Next
                End If
        End Select
    ...
    Private Function AddUpdateGO(ByVal Tx As TxRecord, Optional ByVal rowposition As Integer = 0) As Boolean
        Dim myret As Boolean = True
        ...
        Select Case Tx
            Case TxRecord.TxAdd
                Dim bsdetailcurrent As DataRowView = bsDetail.Current
                mydrv = bsGO.AddNew
                mydrv.Item("otherdate") = Today.Date
                mydrv.Item("documentid") = bsdetailcurrent.Item("documentid")
                mydrv.Item("vdid") = bsdetailcurrent.Item("vdid")
                mydrv.Item("status") = True
            Case TxRecord.TxUpdate
                mydrv = DataGridView4.Rows(rowposition).DataBoundItem
        End Select
        Dim myform As New DialogGeneralContractOther(mydrv, bsPaymentTerm)
        If myform.ShowDialog = Windows.Forms.DialogResult.OK Then
            mydrv.EndEdit()
        Else
            mydrv.CancelEdit()
    ...
    Private Sub DataGridView4_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        Select Case CType(sender, DataGridView).Name
            Case "DataGridView4"
                AddUpdateGO(TxRecord.TxUpdate, e.RowIndex)
            Case "DataGridView5"
                AddUpdateSO(TxRecord.TxUpdate, e.RowIndex)
            Case "DataGridView6"
                AddUpdateQO(TxRecord.TxUpdate, e.RowIndex)
    ...
    Private Function AddUpdateSO(ByVal Tx As TxRecord, Optional ByVal rowposition As Integer = 0) As Boolean
        Dim myret As Boolean = True
        Dim mydrv As DataRowView = Nothing
        ...
        Select Case Tx
            Case TxRecord.TxAdd
                ...
                mydrv.Item("documentid") = bsdetailcurrent.Item("documentid")
                mydrv.Item("vdid") = bsdetailcurrent.Item("vdid")
                mydrv.Item("leadtimehd") = "Lead Time"
                mydrv.Item("saslhd") = "SASL"
                mydrv.Item("status") = True
            Case TxRecord.TxUpdate
                mydrv = DataGridView5.Rows(rowposition).DataBoundItem
        End Select
        Dim myform As New DialogSupplyChainOther(mydrv)
        If myform.ShowDialog = Windows.Forms.DialogResult.OK Then
            mydrv.EndEdit()
        Else
            mydrv.CancelEdit()
    ...
    Private Function AddUpdateQO(ByVal Tx As TxRecord, Optional ByVal rowposition As Integer = 0) As Boolean
        Dim myret As Boolean = True
        Dim mydrv As DataRowView = Nothing
        ...
        Select Case Tx
            Case txRecord.TxAdd
                ...
                mydrv.Item("status") = True
            Case txRecord.TxUpdate
                mydrv = DataGridView6.Rows(rowposition).DataBoundItem
        End Select
        Dim myform As New DialogQualityAppendixOther(mydrv)
        If myform.ShowDialog = Windows.Forms.DialogResult.OK Then
            mydrv.EndEdit()
        Else
            mydrv.CancelEdit()
    ...
    Private Sub Button13_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImportBuyerInput.Click, ImportActionPlan.Click
        Dim mybutton As Button = DirectCast(sender, Button)
        If ShortNameTMP = "" Then
            MessageBox.Show("Short Name is mandatory.")
            Exit Sub
        End If
        Dim myValidatorBS As New BindingSource
        myValidatorBS.DataSource = DS.Tables(8)
        Dim getValidator As New DialogValidator(myValidatorBS)
        myValidatorBS.Filter = "teamtitleshortname <> 'PO'"
        ' If getValidator.ShowDialog = Windows.Forms.DialogResult.OK Then
        Dim ImportFile = New OpenFileDialog
        ImportFile.FileName = "Select File"
        If ImportFile.ShowDialog = Windows.Forms.DialogResult.OK Then
            Dim mydrv As DataRowView = bsDetail.Current
            Dim myobj As Object
            If mybutton.Name = "ImportBuyerInput" Then
                myobj = New ClassImportBuyerInput(mydrv, ImportFile.FileName)
            Else
                getValidator.ShowDialog()
                myobj = New ClassImportActionPlan(bsActionPlan, mydrv, ImportFile.FileName, getValidator.validator)
                If ShortNameTMP.Length = 0 Then
                    ShortNameTMP = mydrv.Item("shortname")
                End If
                myobj.shortname = ShortNameTMP
            End If
            mydrv.EndEdit()
            If Not myobj.getrecord() Then
                MessageBox.Show(myobj.errormessage)
