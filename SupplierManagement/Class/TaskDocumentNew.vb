...
Public Class TaskDocumentNew
    ...
    Public Function getBodyMessage(ByVal data As Object) As String Implements IDocumentTask.getBodyMessage
        ...
        Dim validatorname As String = DirectCast(data(0), DataRowView).Item("validatorname")
        sb.Append("<!DOCTYPE html><html><head><meta name=""description"" content=""[SupplierManagement]"" /><meta http-equiv=""Content-Type"" content=""text/html; charset=us-ascii""></head><style>  td,th {padding-left:5px;         padding-right:10px;         text-align:left;  }  th {background-color:red;    color:white}  .defaultfont{    font-size:11.0pt; font-family:""Calibri"",""sans-serif"";    }</style><body class=""defaultfont""><p>Dear " & validatorname & ",</p>" &
                  ...
                  "<tr><th>Creation Date</th><th>User Name</th> <th>Short Name</th> <th>Supplier(s)</th> <th>Supplier Code</th><th>Document Type(s)</th><th>Project/Product Name</th><th>Version</th><th>Document Date</th><th>Expired Date</th><th>Status</th><th>Validator</th><th>CC</th></tr>")
        For Each n As DataRowView In data
            sb.Append(String.Format("<tr><td>{0:dd-MMM-yyyy}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8:dd-MMM-yyyy}</td><td>{9::dd-MMM-yyyy}</td><td>{10}</td><td>{11}</td><td>{12} {13} {14} {15}</td></tr>", n.Item("creationdate"), n.Item("creatorname"), n.Item("shortname"), n.Item("suppliername"), n.Item("vendorcode"), n.Item("doctypename"), n.Item("projectname"), n.Item("version"), n.Item("docdate"), n.Item("expireddate"), "NEW", n.Item("validatorname"), n.Item("cc1name"), n.Item("cc2name"), n.Item("cc3name"), n.Item("cc4name")))
        Next
    ...
    Public Function getQuery() As Object Implements IDocumentTask.getQuery
        ...
        'Return From n In bs.List
        '                 Group n By test = New With {Key .validator = n.item("validator"), Key .cc = n.item("cc")} Into Group
        '                 Select test, data = Group
    End Function
End Class
