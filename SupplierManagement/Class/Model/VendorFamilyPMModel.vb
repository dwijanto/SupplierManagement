...
Public Class VendorFamilyPMModel
    ...
    Public Function LoadData(ByVal DS As DataSet) As Boolean Implements IModel.LoadData
        ...
        Using conn As Object = myadapter.getConnection
            conn.Open()
            Dim sqlstr = String.Format("select vf.id,vf.familyid,vf.vendorcode::text,vf.pmeffectivedate,vf.spmeffectivedate,f.familyname::text,case when vf.familyid = 0  then 'To Be Define' else f.familyid || ' - ' || f.familyname end as familydesc,case when vf.familyid = 0  then 'To Be Define' else mu.username end as pm,case when vf.familyid = 0  then 'To Be Define' else mus.username end as spm,v.vendorname::text,v.shortname3::text as shortname  from doc.vendorfamilyex vf " &
