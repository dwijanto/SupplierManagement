...
Public Class NSEmail
    ...
    Public Function Execute(ByVal sendto As String, ByVal sendtoname As String, ByVal statusname As String, ByVal drv As DataRowView, Optional ByVal cc As String = "") As Boolean
        ...
        Try
            ...
            Me.subject = String.Format("Vendor Creation: Tasks status. ({0:dd-MMM-yyyy}).", Today.Date)
    ...
    Private Function getBodyMessage() As String
        ...
        sb.Append("<!DOCTYPE html><html><head><meta name=""description"" content=""[SupplierManagement]"" /><meta http-equiv=""Content-Type"" content=""text/html; charset=us-ascii""></head><style>  td,th {padding-left:5px;         padding-right:10px;         text-align:left;  }  th {background-color:red;    color:white}  .defaultfont{    font-size:11.0pt; font-family:""Calibri"",""sans-serif"";    }</style><body class=""defaultfont""><p>Dear " & sendtoname & ",</p>" &
                  " <p>Please be informed that we have the following Vendor Creation tasks need to follow up:<br><br>List of Tasks:</p>  <table border=1 cellspacing=0 class=""defaultfont"">" &
                  "<tr><th>Status</th><th>Applicant Date</th> <th>Applicant Name</th> <th>Vendor Code</th> <th>Vendor Name</th><th>Short Name</th><th>Creator</th><th>Vendor Creation ID</th></tr>")

        sb.Append(String.Format("<tr><td>{0}</td><td>{1:dd-MMM-yyyy}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td></tr>", statusname, drv.Item("applicantdate"), drv.Item("applicantname"), drv.Item("vendorcode"), drv.Item("vendorname"), drv.Item("shortname"), drv.Item("creator"), drv.Item("suppliermodificationid")))
