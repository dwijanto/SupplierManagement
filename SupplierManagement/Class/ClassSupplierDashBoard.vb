...
Public Class ClassSupplierDashBoard
    ...
    Sub DoWork()
        If HelperClass1.UserInfo.IsAdmin Then
            sb.Append("select null::text as shortname union all (select distinct shortname3::text as shortname from vendor  where not shortname3 isnull order by shortname3);")
            sb.Append("select null as vendorcode,''::text as description,''::text as vendorname,null::text as shortname,null::text as shortname2 union all (select vendorcode, vendorcode::text || ' - ' || vendorname::text as description,vendorname::text,shortname3::text as shortname,shortname2::text  from vendor order by vendorname);")
        ...
        Else
            sb.Append(String.Format("select null::text as shortname union all  (select distinct shortname3::text as shortname from doc.groupvendor gv left join vendor  v on v.vendorcode = gv.vendorcode left join doc.groupauth g on g.groupid = gv.groupid" &
                 " left join doc.groupuser gu on gu.groupid = gv.groupid left join doc.user u on u.id = gu.userid where u.userid ~ '{0}$' and not shortname3 isnull order by shortname3);", HelperClass1.UserInfo.userid))
            sb.Append(String.Format("select null as vendorcode,''::text as description,''::text as vendorname,null::text as shortname,null::text as shortname2 union all (select distinct v.vendorcode, v.vendorcode::text || ' - ' || v.vendorname::text as description,v.vendorname::text,shortname3::text as shortname,shortname2::text  from doc.groupvendor gv left join vendor  v on v.vendorcode = gv.vendorcode left join doc.groupauth g on g.groupid = gv.groupid" &
